...
//    sprintf(fname, argv[1]);
    ...
    int arr_Iter[] = {1, 10, 100};
    int maxMatchCount = 0;
    int maxHMMNumber = 0;

    for(int z = 0 ; z < 3 ; z++){
        for(int hmm_no = 0 ; hmm_no < arr_Iter[z] ; hmm_no++){
            ...
            printf("\n HMM number %d and match count %d", hmm_no, maxMatchCount);
        }
    }
    printf("\n The max match was of HMM %d with match count %d\n\n", maxHMMNumber, maxMatchCount);
}// end hmm
